...
describe('L9 · ROUTING · per-entity RelativePath', () => {
  before(startServer);
  after(stopServer);

  for (const e of ROUTING.relativePaths) {
    test(`${e.endpoint} · ${e.field} = ${e.expected}`, async () => {
...
describe('L9 · ROUTING · role-based nav trees', () => {
  before(startServer);
  after(stopServer);

  for (const [role, expectedTops] of Object.entries(ROUTING.navTrees)) {
    test(`role ${role} sees top-level routes: ${expectedTops.join(', ')}`, async () => {
