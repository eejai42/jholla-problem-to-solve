...
INSERT INTO leopold_loops (leopold_loop_id, loop_number, title, goal, status, rule_commit_msg, state_commit_msg, sort_order, status_badge, status_line)
VALUES ('loop-3', '3', 'Gate explainability in the UI', 'Write path SHIPPED: POST /api/intake writes a new Individual + full leaf chain (variant, assays, mechanism, evidence, replications, controls, intervention targets, prediction, calibration bins) to base tables in one transaction, then re-reads the derived panel from vw_*. DELETE /api/individuals/:id cleans up. Witnessed by L10 harness: raw leaves -> derived ACTIONABLE keystone with no conclusion entered; one knob flip flips the verdict; a smuggled conclusion is ignored. 322/322 green.', 'done', 'rule: install explainer-dag', 'state: Loop 3 - facts-in to derived diagnosis works end to end (322/322)', 4, '[DONE]', '') ON CONFLICT (leopold_loop_id) DO NOTHING;
