...
KINDS = [
    ...
    dict(
        ...
        maturity="Partial",
        evidence="closure field carries owl:TransitiveProperty intent; OWL-RL == Postgres receipt is the open CR step",
        description=(
            "The same declared model is projected to a second substrate (OWL) and a reasoner closes it "
            "independently; agreement between the OWL-RL closure and the Postgres recursive view is the "
            "conformance receipt. The Postgres closure is live; the OWL projection / referee is the "
            "in-flight CR step, so this is marked Partial."
        ),
        example="rulebook-to-owl projection of ProgressionClosure (reason.py closure == vw_*_closure)",
    ),
]
